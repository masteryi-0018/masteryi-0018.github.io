...
3. 如果分步进行，将iso内容复制（不同于dd的操作），然后手动加boot loader（grub）
```sh
# 复制文件
sudo mount /dev/sda1 ubuntu_root
sudo mount -o loop ubuntu.iso iso_mount
sudo cp -r iso_mount/* ubuntu_root/

# 安装grub
sudo apt install grub-pc
sudo grub-install --target=i386-pc --boot-directory=ubuntu_root/boot /dev/sda
